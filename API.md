...
## Additional Feature 1: Search notes by modified date/created date (The API has the functionality to search for a range of dates)
Run this with `'search_field': 'created_date'`
You can run and use "modified_date" the exact same way so I will not provide a separate explanation
...
`'start_query'`: string representing the start date of the range of dates you want to search for
`'end_query'`: string representing the end date of the range of dates you want to search for
...
## Additional Feature 6: Search tags 
Search for titles associated for a specified tag
...
`'query'`: string that is the name of the name of the tag

## Additional Feature 7: Markdown conversion 
Converts a python string into a markdown file and saves it locally on your computer 
...
`'title'`(required): A string representing the title of the markdown file you are creating 
`'content'`(required): A string with the content you want to put into the markdown file

## Additional Feature 8: Images
The image functionality allows you to add and delete image paths from a SQL database. I will not add much description to this functionality because it functions almost the exact same way as the tags in the REST API, but I will note key differences. 
